...
- Daily Floor は GitHub Pages-only のライフログPWA。バックエンド、サーバー処理、Firebase、Supabase、外部DB、外部分析、外部ログは禁止。
...
- Daily Floor の中核は、目的作成、今日の1歩の達成、振り返り、日記タイムライン。日記、気分、写真、添付、休養ログも時系列で残す。
- UIでは「床」ではなく `floor` と書く。書けない日、短い記録、休養ログはいずれも有用なデータとして扱う。
- 睡眠不足や低エネルギー時は目標を下げる。通常量を押し付けない。
- 医療診断、服薬助言、治療判断、躁/うつ等の断定は禁止。出せるのは事実ログ、共有候補、中立フラグだけ。
- 主要導線は「今日」「ジャーナル」「More」。ボタンは押した後に何が起きるかを明確にし、保存後は必ずフィードバックを表示する。
- ガイド画像は外部スクリーンショットのコピーにしない。実アプリと一致する説明、またはアプリ内UIで作ったプレビューを使う。
